...
void FFIChain::ConfigureAcc() {
	...
	hu_audiodec_cfg_000[0].esp.coherence = CoherenceMode;

	hu_audiodec_cfg_000[0].cfg_regs_2 = m_nBlockSize;
	hu_audiodec_cfg_000[0].src_offset = 10 * acc_len * sizeof(device_t);
	hu_audiodec_cfg_000[0].dst_offset = ((10 * acc_len) + (m_nChannelCount * m_nBlockSize)) * sizeof(device_t);
...
void FFIChain::PsychoRegularProcess(CBFormat* pBFSrcDst, kiss_fft_cpx** m_Filters, audio_t** m_pfOverlap) {
	...
	// Emulating Regular with ASI: set the end flags so that the accelerators gracefully end in every
	// iteration. We test the consumer flags (FFT and FIR) before writing the input data. We set the 
	// valid flags before running esp_run (with start_stop disabled)
	while (ChannelsLeft != 0) {
...
void FFIChain::OffloadRotateOrder(CBFormat* pBFSrcDst) {
	...
	StartCounter();
	for (unsigned niSample = 0; niSample < InitLength; niSample++)
	{
